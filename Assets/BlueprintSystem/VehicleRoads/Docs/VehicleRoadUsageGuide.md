...
- `BT.VehicleRoad.DriveFollower`：便捷封装节点；从 Blackboard 或 owner GameObject 解析 `VehicleLaneFollower`，维护内部当前速度，按 `VehicleRoadTestVehicle` 的加减速、停止点夹紧、baked route pose 和 loop reset 逻辑移动 owner，额外写 `currentSpeedKey`、`arrivedKey` 和 `loopResetKey`。如果停止线已经在车后，或 `stopPoint` 位于 owner 当前 forward 后方，则跳过停止点吸附，继续走 baked route / look-ahead fallback movement。
...
- `BT.VehicleRoad.UpdateFollowerSpeed` / `EvaluateStopPointTravel` / `ApplyStopPoint`：拆分 `VehicleRoadTestVehicle.Update` 中的速度更新、停止点行驶距离计算和停止点吸附。`EvaluateStopPointTravel` 会保留已越过停止线时的正常前进距离并写 `reachedStopPointKey=false`；`ApplyStopPoint` 在停止点位于车后时返回 `Failure`，让 Selector 继续尝试后续移动节点。
