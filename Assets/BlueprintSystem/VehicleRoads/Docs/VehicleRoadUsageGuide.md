...
- `BT.VehicleRoad.UpdateFollowerSpeed` / `EvaluateStopPointTravel` / `ApplyStopPoint`：拆分 `VehicleRoadTestVehicle.Update` 中的速度更新、停止点行驶距离计算和停止点吸附。`EvaluateStopPointTravel` 在停止线前会按 `stopPointDecelerationTime` / `maxStopPointDeceleration` 写回 `currentSpeedKey` 和 `speedChangeKey`；已越过停止线时保留正常前进距离，写 `reachedStopPointKey=false` 且 `speedChangeKey=0`。`ApplyStopPoint` 在停止点位于车后时返回 `Failure`，让 Selector 继续尝试后续移动节点。
