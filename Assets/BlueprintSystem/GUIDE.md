...
Behavior Tree-native road nodes live in the Behavior Tree registry, not in `.node.json` manifests: `BT.VehicleRoad.FindNearestLane`, `BT.VehicleRoad.FindLaneRoute`, `BT.VehicleRoad.ComputeFollowerControl`, `BT.VehicleRoad.DriveFollower`, and `BT.VehicleRoad.UpdateRoadAgent`. Query/control-output nodes write Blackboard keys and do not move GameObjects; `BT.VehicleRoad.DriveFollower` is the explicit kinematic owner-movement task for demo/AI vehicles. Detailed authoring rules, port lists, and runtime ownership boundaries live in `Assets/BlueprintSystem/VehicleRoads/Docs/VehicleRoadUsageGuide.md` and `Assets/BlueprintSystem/BehaviorTree/GUIDE.md`.
