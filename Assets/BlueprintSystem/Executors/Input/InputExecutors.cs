...
namespace BlueprintSystem
{
    ...
    [Obsolete("Input.ListenKey and Input.ListenAction now poll when executed. This host remains only for legacy callers.")]
    [DisallowMultipleComponent]
    [AddComponentMenu("")]
    public sealed class BlueprintInputListenerHost : MonoBehaviour
    {
        private readonly List<InputListener> _listeners = new List<InputListener>();

        public static BlueprintInputListenerHost GetOrAdd(BlueprintExecutionContext context)
        {
            GameObject owner = ResolveOwner(context);
            if (owner == null)
            {
                return null;
            }

            BlueprintInputListenerHost host = owner.GetComponent<BlueprintInputListenerHost>();
            return host != null ? host : owner.AddComponent<BlueprintInputListenerHost>();
        }

        public void RegisterKey(BlueprintExecutionContext context, RuntimeNode node, Key key)
        {
        ...
        public void RegisterAction(BlueprintExecutionContext context, RuntimeNode node, InputAction action)
        {
            AddListener(new InputListener
            {
                Id = "action:" + node.Id + ":" + action.id,
                Context = context,
                Node = node,
                IsPressed = delegate { return BlueprintInputPollingUtility.IsActionPressed(action); }
            });
        }
        ...
        private void Poll()
        {
            ...
            {
                InputListener listener = _listeners[i];
                if (listener == null || listener.Context == null || listener.Node == null)
                {
                    continue;
                }
                ...
                try
                {
                    isPressed = listener.IsPressed != null && listener.IsPressed();
                }
                catch (Exception exception)
                {
                    listener.Context.Logger.Warning("Input listener '" + listener.Id + "' failed: " + exception.Message);
                    continue;
                }

                if (isPressed)
                {
                    if (!listener.WasPressed)
                    {
                        listener.WasPressed = true;
                        listener.Context.ExecuteFromOutput(listener.Node, "pressed");
                    }
                    else
                    {
                        listener.Context.ExecuteFromOutput(listener.Node, "held");
                    }
                }
                else if (listener.WasPressed)
                {
                    listener.WasPressed = false;
                    listener.Context.ExecuteFromOutput(listener.Node, "released");
                }
            }
        }

        private void AddListener(InputListener listener)
        {
            if (listener == null || string.IsNullOrEmpty(listener.Id))
            {
                return;
            }
            ...
            for (int i = 0; i < _listeners.Count; i++)
            {
                if (_listeners[i] != null && _listeners[i].Id == listener.Id && ReferenceEquals(_listeners[i].Context, listener.Context))
                {
                    return;
                }
            }

            listener.WasPressed = listener.IsPressed != null && listener.IsPressed();
            _listeners.Add(listener);
        }

        private static GameObject ResolveOwner(BlueprintExecutionContext context)
        {
            if (context == null)
            {
                return null;
            ...
            return context.OwnerComponent == null ? null : context.OwnerComponent.gameObject;
        }

        private sealed class InputListener
        {
            public string Id;
